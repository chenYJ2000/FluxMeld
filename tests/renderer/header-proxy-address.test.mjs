...
test('header proxy badge renders the configured bind address instead of a hard-coded loopback host', () => {
  const headerSource = readFileSync(
    join(root, 'src/renderer/src/components/layout/Header.tsx'),
    'utf8',
  )
...
test('tool calling smoke uses a local management API URL that is actually served by the proxy', () => {
  ...
  const managementSettingsSource = readFileSync(
    join(root, 'src/renderer/src/components/settings/ManagementApiSettings.tsx'),
    'utf8',
  )
  ...
  assert.match(
    managementSettingsSource,
    /const apiEndpoint = `\$\{window\.location\.origin\}\/v0\/management`/,
  )
  assert.match(managementSettingsSource, /window\.electronAPI\?\.platform === 'web'/)
  assert.doesNotMatch(managementSettingsSource, /127\.0\.0\.1/)
})
