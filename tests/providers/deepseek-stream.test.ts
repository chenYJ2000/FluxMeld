...
test('DeepSeek stream appends citations from HAR fragment results', async () => {
  ...
  const source = sse([
    ...
    },
    {
      ...
      o: 'APPEND',
      v: [{ id: 3, type: 'RESPONSE', content: '北京明天天气多云。' }],
    },
  ])
  ...
  assert.match(
    joined,
    /\[1\]: \[北京-天气预报\]\(https:\/\/www\.nmc\.cn\/publish\/forecast\/ABJ\/beijing\.html\)/,
  )
...
test('DeepSeek stream keeps existing cite index when merging duplicate URL without cite index', async () => {
  ...
  const source = sse([
    ...
    },
    {
      p: 'response/fragments',
      o: 'APPEND',
      v: [{ id: 4, type: 'RESPONSE', content: '引用来源已更新。' }],
    },
  ])
...
test('DeepSeek stream keeps existing cite index when duplicate URL has invalid cite index', async () => {
  ...
  const source = sse([
    ...
    },
    {
      p: 'response/fragments',
      o: 'APPEND',
      v: [{ id: 8, type: 'RESPONSE', content: '引用索引保持稳定。' }],
    },
  ])
...
test('DeepSeek stream normalizes camelCase citeIndex search results', async () => {
  ...
  const source = sse([
    ...
    },
    {
      p: 'response/fragments',
      o: 'APPEND',
      v: [{ id: 5, type: 'RESPONSE', content: '引用格式正常。' }],
    },
  ])
...
test('DeepSeek search-silent stream suppresses citations', async () => {
  const handler = new DeepSeekStreamHandler(
    'deepseek-v4-flash-search-silent',
    'session-silent',
    undefined,
    true,
  )
  const source = sse([
    ...
    },
    {
      p: 'response/fragments',
      o: 'APPEND',
      v: [{ id: 7, type: 'RESPONSE', content: '不会附加引用。' }],
    },
  ])
...
test('DeepSeek non-stream applies batched cite index patches to search results', async () => {
  ...
  const source = sse([
    ...
    { p: 'response/fragments/-1/results', o: 'BATCH', v: [{ p: '0/cite_index', v: 1 }] },
    {
      p: 'response/fragments',
      o: 'APPEND',
      v: [{ id: 9, type: 'RESPONSE', content: '批量引用已生成。' }],
    },
  ])
...
test('DeepSeek non-stream keeps existing cite index when duplicate URL has invalid cite index', async () => {
  const handler = new DeepSeekStreamHandler(
    'deepseek-v4-flash-search',
    'session-nonstream-invalid-cite',
    undefined,
    true,
  )
  const source = sse([
    ...
    },
    {
      p: 'response/fragments',
      o: 'APPEND',
      v: [{ id: 10, type: 'RESPONSE', content: '引用索引稳定。' }],
    },
  ])
...
test('DeepSeek non-search streams preserve search text at chunk start', async () => {
  const handler = new DeepSeekStreamHandler(
    'deepseek-v4-flash',
    'session-preserve-stream-search',
    undefined,
    false,
  )
  const source = sse([
    { v: { response: { thinking_enabled: false } } },
    {
      p: 'response/fragments',
      o: 'APPEND',
      v: [{ id: 12, type: 'RESPONSE', content: 'search.example.com' }],
    },
  ])
