  ...
  refreshData: async () => {
    ...
    try {
      ...
      const statisticsPromise =
        window.electronAPI?.invoke?.('proxy:getStatistics') ?? Promise.resolve(null)
      const persistentStatsPromise =
        window.electronAPI?.statistics?.get?.() ?? Promise.resolve(null)
      ...
      const providerStatusPromise =
        window.electronAPI?.providers?.checkAllStatus?.() ?? Promise.resolve({})
      ...
      const requestLogTrendPromise =
        window.electronAPI?.requestLogs?.getTrend?.(7) ?? Promise.resolve([])
      ...
      const requestLogsPromise =
        window.electronAPI?.requestLogs?.get?.({ limit: 100 }) ?? Promise.resolve([])
