    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1 text-sm">
          {entry.provider && <span className="font-medium">{entry.provider}</span>}
          {entry.model && <span className="text-muted-foreground">/ {entry.model}</span>}
        </div>

        {entry.userInput && (
          <p className="text-xs text-muted-foreground truncate mt-1">{entry.userInput}</p>
        )}

        {entry.error && <p className="text-xs text-red-500 truncate mt-1">{entry.error}</p>}
      </div>

      <div className="flex items-center gap-4 text-xs text-muted-foreground flex-shrink-0">
        {entry.duration && <span>{(entry.duration * 1000).toFixed(0)}ms</span>}
