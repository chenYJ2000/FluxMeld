...
const MODEL_DEPRECATIONS: Record<string, ModelDeprecation> = {
  'glm-5.1': {
    ...
    replacement: 'GLM-5.3',
    message:
      'GLM-5.1 is no longer available from the GLM web provider. GLM-5.3 has different behavior and is not selected automatically. Validate it first, then add an explicit model mapping from GLM-5.1 to GLM-5.3 if desired.',
  },
}
