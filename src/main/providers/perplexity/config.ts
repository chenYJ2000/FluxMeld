...
export const perplexityConfig: BuiltinProviderConfig = {
  ...
  credentialFields: [
    {
      name: 'sessionToken',
      labelKey: 'perplexity.sessionToken',
      placeholderKey: 'perplexity.sessionTokenPlaceholder',
      helpTextKey: 'perplexity.sessionTokenHelp',
      label: 'Session Token',
      type: 'password',
      required: true,
      placeholder: 'Enter Perplexity session token',
      helpText:
        'Session token obtained from Perplexity web version (__Secure-next-auth.session-token cookie)',
    },
  ],
  capabilities: { clearChats: true },
  ui: {
    iconKey: 'perplexity',
    i18nPrefix: 'perplexity',
    noticeKey: 'perplexity.freeUserNotice',
  },
}
