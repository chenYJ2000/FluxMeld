...
function parseStoredCookies(raw?: string): StoredCookies {
  if (!raw) return {}
  try {
    ...
    // Not JSON; treat as empty rather than throwing at construction time.
  }
  return {}
}
...
export class PerplexityAdapter {
  ...
  private buildRequestData(query: string, model: string): any {
  ...
  updateSessionData(data: Partial<SessionData>): void {
    ...
    const newData: SessionData = {
      ...
      frontend_context_uuid:
        data.frontend_context_uuid || existing?.frontend_context_uuid || uuid(),
