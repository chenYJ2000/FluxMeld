...
export const minimaxConfig: BuiltinProviderConfig = {
  ...
  credentialFields: [
    {
      name: 'token',
      labelKey: 'minimax.token',
      placeholderKey: 'minimax.tokenPlaceholder',
      helpTextKey: 'minimax.tokenHelp',
      label: 'JWT Token',
      type: 'password',
      required: true,
    ...
    },
    {
      ...
      helpTextKey: 'minimax.realUserIDHelp',
      label: 'Real User ID (Optional)',
      type: 'text',
      required: false,
      placeholder: 'Enter Real User ID (optional)',
      helpText:
        'If provided, use this instead of JWT user ID. Can also use format: realUserID+JWTtoken in token field',
    },
  ],
